...
function populateRow(containerId, movies) {
    ...
    if (!movies || movies.length === 0) {
        container.innerHTML = '<p style="color: #9ca3af; padding-left: 10px;">No movies available right now.</p>';
        return;
    }
    ...
    container.innerHTML = movies.map(movie => `
        <div class="movie-card" onclick="window.location.href='index.html?movie_id=${movie.id}'">
            ...
            <div class="movie-card-info" style="padding: 10px 0;">
                <h3 style="margin: 0 0 5px 0; font-size: 15px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${movie.title}</h3>
